      ...
      <div className="overflow-x-auto">
        ...
        <thead>
          <tr className="bg-gradient-to-r from-slate-50 via-blue-50/50 to-violet-50/50 dark:from-slate-800/80 dark:via-blue-900/20 dark:to-violet-900/20 border-b-2 border-slate-200 dark:border-slate-700">
            <th className="px-6 py-5 text-slate-700 dark:text-slate-300 text-xs font-extrabold uppercase tracking-widest w-[35%]">
              <div className="flex items-center gap-2">
                <span className="material-symbols-outlined text-lg text-blue-600 dark:text-violet-400">person</span>
                Khách hàng
              </div>
            </th>
            <th className="px-6 py-5 text-slate-700 dark:text-slate-300 text-xs font-extrabold uppercase tracking-widest w-[20%]">
              <div className="flex items-center gap-2">
            ...
            <th className="px-6 py-5 text-slate-700 dark:text-slate-300 text-xs font-extrabold uppercase tracking-widest w-[18%]">
              <div className="flex items-center gap-2">
                <span className="material-symbols-outlined text-lg text-violet-600 dark:text-violet-400">monitoring</span>
                Trạng thái
              </div>
            </th>
            <th className="px-6 py-5 text-slate-700 dark:text-slate-300 text-xs font-extrabold uppercase tracking-widest text-right w-[7%]">
              Thao tác
            </th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
              ...
              >
                <td className="px-6 py-5">
                  <div className="flex items-center gap-4">
                    <div
                      className={`size-11 rounded-xl flex items-center justify-center font-bold text-base shadow-md ${lead.avatarColorClass} ${isSelected ? 'scale-110 shadow-lg' : 'group-hover:scale-105'} transition-transform duration-300`}
                    >
                      {lead.initials}
                    </div>
                    <div>
                      <p className="text-slate-900 dark:text-white font-bold text-base mb-0.5">
                        {lead.name}
                      </p>
          ...
          {Array.from({ length: emptyRowsCount }).map((_, index) => (
            <tr key={`empty-${index}`} className="border-l-4 border-l-transparent">
              <td className="px-6 py-5" style={{ height: '85px' }}>
